  ...
  return (
    <main className="min-h-screen bg-black text-white flex flex-col items-center p-8">
      ...
      <div className="flex gap-4 mb-8">
        <button
          onClick={() => setTab("send")}
          className={tab === "send" ? "px-6 py-2 rounded-full font-semibold bg-purple-600 text-white" : "px-6 py-2 rounded-full font-semibold bg-gray-800 text-gray-400"}
        >
          Send File
        </button>
        <button
          onClick={() => setTab("receive")}
          className={tab === "receive" ? "px-6 py-2 rounded-full font-semibold bg-purple-600 text-white" : "px-6 py-2 rounded-full font-semibold bg-gray-800 text-gray-400"}
        >
          Receive File
        </button>
      </div>
        ...
        <div className="w-full max-w-md bg-gray-900 rounded-2xl p-6 flex flex-col gap-4">
          ...
          {txHash && (
            <div className="text-xs bg-gray-800 rounded-lg p-3 break-all">
              <p className="text-gray-300 mb-1 font-semibold">Transaction Hash:</p>
              <a href={getExplorerLink()} target="_blank" className="text-purple-400">
                {txHash}
              </a>
            </div>
          )}
